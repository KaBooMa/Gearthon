...
class ScriptBehaviour : GearLib.API.Behaviour
{    
    ...
    public void SetupTweakables() {
        ...
        JArray tweakables;
        Plugin.tweakables.TryGetValue(uid, out tweakables);
        if (tweakables == null)
            return;

        for (int i2 = 0; i2 < tweakables.Count; i2++)
        {
            JToken tweakable_data = tweakables[i2].Cast<JToken>();
